...
### 语音（`claw/services/voice/`）

可选语音服务，保持现有 AI 主链不变：输入后端向 `voice_service` 提交
`start_capture` / `audio_chunk` / `end_capture` 事件，STT 将音频转成文本后，
再把 transcript 送入 `ai_chat()`。`ai_chat()` 返回的 assistant 文本同时回传给
端点，并继续送入 TTS；TTS 解码后的音频再交还给当前输出后端。当前实现包含
STT/TTS 的配置驱动 provider 选择、单轮字节硬截断、音频格式元数据、基于队列的
事件交接、带缓冲回退的 MiMo TTS 响应流式处理，以及位于
`platform/linux/web_voice_server.c` 的 Linux web 端点后端。Linux web 路径使用
浏览器 HTTP POST 上传 PCM，并通过 SSE 回传状态、transcript、assistant 文本和
TTS 音频更新。

在接口层，`include/claw/services/voice/voice_endpoint.h` 定义了 session 状态、
端点事件类型以及后端回调契约。采集侧后端通过 `voice_submit_event()` 提交
`ATTACH`、`DETACH`、`START_CAPTURE`、`AUDIO_CHUNK`、`END_CAPTURE`、
`CANCEL` 和 `PLAYBACK_DONE` 事件。`AUDIO_CHUNK` 使用指针型负载
（`data_ptr`/`data_len`）和 `data_owns` 所有权转移语义，因此大块 PCM 数据
不会内联拷贝进队列项。输出侧回调（`send_state`、`send_transcript`、
`send_assistant_text`、`send_tts_audio`、`send_error`）采用 borrow-only 约定：
异步后端如果要延后使用这些数据，必须在返回前自行拷贝。
