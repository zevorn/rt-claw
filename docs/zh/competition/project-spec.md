...
项目灵感来自 OpenClaw 社区，采用纯 C99 实现，通过操作系统抽象层（OSAL）实现跨 RTOS 运行，目前支持 FreeRTOS、RT-Thread 和 Zephyr 三大主流嵌入式操作系统，覆盖 ESP32-C3、ESP32-S3、Zynq-A9、vexpress-A9 及 Zephyr qemu_cortex_a9/m3 等平台。
...
**绑定方式**：链接时选择 OSAL 实现文件（`claw_os_freertos.c`、`claw_os_rtthread.c` 或 `claw_os_zephyr.c`），零 `#ifdef` 污染。
...
| 模块 | 状态 | 说明 |
|------|------|------|
| OSAL 抽象层 | ✅ | FreeRTOS + RT-Thread + Zephyr 三实现 |
...
| Zephyr 支持 | ✅ | QEMU Cortex-A9 + Cortex-M3 |
...
| RTOS | FreeRTOS v11 + RT-Thread v5.3.0 + Zephyr v4.4.0 |
...
5. **跨 RTOS**：同一代码库支持 FreeRTOS + RT-Thread + Zephyr，覆盖主流嵌入式生态
